...
class ChecklistDraftManager(
    ...
    /**
     * This list's database id once it has been persisted, or null while it is
     * still purely in memory. Used by callers that must read the stored list back
     * (e.g. building an export).
     */
    fun persistedId(): Long? = if (_persisted.value) listDbId else null
