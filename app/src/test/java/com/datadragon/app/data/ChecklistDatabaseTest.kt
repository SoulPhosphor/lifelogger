...
class ChecklistDatabaseTest {
    ...
    @Test
    fun migration10To11PreservesFormsAndDefaultsToNewestFirst() {
        val context = ApplicationProvider.getApplicationContext<Context>()
        val configuration = SupportSQLiteOpenHelper.Configuration.builder(context)
            .name(null)
        ...
        val sqlite = FrameworkSQLiteOpenHelperFactory().create(configuration).writableDatabase
        sqlite.execSQL(
            "INSERT INTO log_templates (uuid, name, createdAt, schemaJson, sortTimestampLabel) " +
                "VALUES ('stable-id', 'Mood', 300, '[]', 'Woke Up')"
        )

        AppDatabase.MIGRATION_10_11.migrate(sqlite)

        sqlite.query(
            "SELECT uuid, name, sortTimestampLabel, sortNewestFirst FROM log_templates"
        ).use { cursor ->
            assertTrue(cursor.moveToFirst())
            assertEquals("stable-id", cursor.getString(0))
            assertEquals("Mood", cursor.getString(1))
            assertEquals("Woke Up", cursor.getString(2))
